...
def _make_mock_browser():
    """Create a fully mocked browser with correct async + is_connected behavior."""
    ...
    mock_pw_instance.chromium.launch_persistent_context = AsyncMock(return_value=mock_browser)
    mock_page.title = AsyncMock(return_value="about:blank")
    mock_page.goto = AsyncMock(return_value=None)
    return mock_pw_instance, mock_browser, mock_page


def _patch_playwright(mock_playwright):
    """Wire the async_playwright patch so `await async_playwright().start()` works."""
...
def test_launch_browser(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        _patch_playwright(mock_playwright)
...
def test_profile_creation(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        _patch_playwright(mock_playwright)
        asyncio.run(browser_pool_manager.get_browser("test_agent"))
...
def test_profile_persistence(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        ...
        browser1 = asyncio.run(browser_pool_manager.get_browser("test_agent"))
        mock_page.evaluate.return_value = "test=1"
        asyncio.run(browser1.browser.close())
        asyncio.run(browser1.playwright.stop())
        ...
        browser2 = asyncio.run(browser_pool_manager.get_browser("test_agent"))
...
def test_agent_isolation(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        _patch_playwright(mock_playwright)
        asyncio.run(browser_pool_manager.get_browser("hermes"))
        asyncio.run(browser_pool_manager.get_browser("claude"))
...
def test_max_instances(browser_pool_manager, tmp_path):
    ...
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        _patch_playwright(mock_playwright)
        asyncio.run(browser_pool_manager.get_browser("agent1"))
        asyncio.run(browser_pool_manager.get_browser("agent2"))
        asyncio.run(browser_pool_manager.get_browser("agent3"))
        with pytest.raises(RuntimeError, match="Max instances"):
            asyncio.run(browser_pool_manager.get_browser("agent4"))


def test_shutdown_all(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        mock_pw, mock_browser, mock_page = _patch_playwright(mock_playwright)
        b1 = asyncio.run(browser_pool_manager.get_browser("agent1"))
        b2 = asyncio.run(browser_pool_manager.get_browser("agent2"))
...
def test_cleanup_profile(browser_pool_manager, tmp_path):
    with patch('parousia.spatial.browser_pool.async_playwright') as mock_playwright:
        ...
        profile_dir = Path(browser_pool_manager.config.profile_dir) / "test_agent"
        assert profile_dir.exists()
        asyncio.run(browser_pool_manager.cleanup_profile("test_agent"))
